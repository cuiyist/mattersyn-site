...
export async function openChemical(entry){
 ...
 if(entry.model3dPath&&window.$3Dmol){
  ...
  let highlight=true;const redraw=()=>{if(!viewer)return;viewer.removeAllLabels();viewer.removeAllShapes();viewer.setStyle({},{stick:{radius:.13,colorfunc:a=>atomColors[a.elem]||'#8c91b0'},sphere:{scale:.25,colorfunc:a=>atomColors[a.elem]||'#8c91b0'}});
  ...
  viewer.setClickable({},true,a=>{const isotope=model.atoms[a.index]?.isotope;info.textContent=(elementNames[a.elem]||a.elem)+' ('+(isotope?String(isotope):'')+a.elem+') · atom '+(a.index+1);});

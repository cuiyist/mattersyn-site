...
async function precursors(host,r,presentation={}){
 ...
 for(const m of chemicals){const card=el('article',undefined,'reader-chemical');card.dataset.materialId=m.id;const entry=chemicalEntry(data,r.record_id,m.id);card.append(el('small',human(m.role)),el('h3',m.name),el('div',entry?.displayFormula||entry?.formula||m.formula||'Source-defined composition','reader-formula'));
  ...
  const charge=presentation.precursor_card_quantities?.[m.id];if(charge)card.append(el('p',charge.label||'Source-reported charge','reader-note'),quantities(Object.entries(charge.quantities||{}).map(([key,q])=>[(charge.source_operation_id?charge.source_operation_id+':':'')+key,q]),false,r));
...
export async function mountReader(main,{materialId,recordId}={}){
 ...
 for(const meta of records){const card=button('',()=>select(meta.record_id),'reader-method-card');card.dataset.recordId=meta.record_id;card.append(el('small',(presentations.records?.[meta.record_id]?.method_label||shortMethod(meta.method)).toUpperCase()),el('strong',meta.title),el('small',String(meta.year||'')));cards.append(card);}selector.append(cards);if(records.length>6){[...cards.children].slice(6).forEach(x=>x.hidden=true);selector.append(button('Show all '+records.length+' methods',e=>{for(const c of cards.children)c.hidden=false;e.currentTarget.hidden=true;},'reader-chip'));}main.append(selector);
